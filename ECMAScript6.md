...
> 注意 :zap: http://es6.ruanyifeng.com/#docs/string
...
Array.of方法用于将一组值，转换为数组。Array方法没有参数、一个参数、三个参数时，返回结果都不一样。只有当参数个数不少于2个时，Array()才会返回由参数组成的新数组。参数个数只有一个时，实际上是指定数组的长度。
```javascript=
...
(function() {
  console.log('kevin');
  loadImageAsync('https://i.imgur.com/ao5kfS6.png').then((img) => {
...
下面代码中，p1和p2都是 Promise 的实例，但是p2的resolve方法将p1作为参数，即一个异步操作的结果是返回另一个异步操作。
...
下面代码中，promises是包含6个 Promise 实例的数组，只有这6个实例的状态都变成fulfilled，或者其中有一个变为rejected，才会调用Promise.all方法后面的回调函数。
```javascript=
// 生成一个Promise对象的数组
...
### 3. String iterator interface

下面代码中，调用Symbol.iterator方法返回一个遍历器对象，在这个遍历器上可以调用next方法，实现对于字符串的遍历。
```javascript=
var someString = "hi";
...
### 4. Iterator interface & Generator function
...
### 5. return() & throw() of object iterator
...
## [Ref : http://es6.ruanyifeng.com/#README](http://es6.ruanyifeng.com/#README)
